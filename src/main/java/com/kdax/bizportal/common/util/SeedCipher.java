...
public class SeedCipher {
	...
	public byte[] encrypt(byte[] sbuffer, byte szKey[]) {
		...
		byte[] inDataBuffer = padding.add80Padding(sbuffer, SeedBlockSize);
	...
	public byte[] decryptImg(byte[] encryptBytes, byte[] szKey, boolean isFinal) {
		int sRoundKey[] = new int[32];
		byte[] decryptBytes = new byte[encryptBytes.length];
		SeedEncRoundKey(sRoundKey, szKey);
		int rt = encryptBytes.length / SeedBlockSize;
	...
	public byte[] getByteFromHexString(String raw) {
		...
		byte[] bReturn = new byte[nLen/2];		
		int nI = 0;
		for(int i = 0; i < nLen; i=i+2){			
			int n = Integer.parseInt(raw.substring(i, i+2), 16);			
			bReturn[nI] = (byte)n;		
			nI++;
		}		
		return bReturn;
	...
	public String encryptData(String inData, byte[] szKey){
		SeedCipher cipher = new SeedCipher();
		
		byte[] byteData = cipher.encrypt(inData, szKey);
		/*
		StringBuffer sb = new StringBuffer(); 
		for(int i = 0 ; i < byteData.length ; i++){
			sb.append(Integer.toString((byteData[i]&0xff) + 0x100, 16).substring(1));
		...
		return sb.toString();*/
		return this.getHex(byteData);
	}
	
	public int setClientKey(byte[] bSmk, byte[] bClientKey){
		if(bClientKey.length > 16){
			return -1;
		}
		if(bSmk.length > 16){
			return -2;
		...
		this.bCmk = encrypt(bClientKey, bSmk);
		return 0;
	}
	...
	public byte[] getCleintKey(byte[] bSmk, byte[] bClientKey){
		if(bClientKey.length > 16){
			return null;
		}
		if(bSmk.length > 16){
	...
	public boolean decryptFile(String key, String path, String destPath){
		...
		try (
			...
			while ((len = bis.read(data)) != -1) {		
		    	...
		    	if(len < size){
		    		byte[] decData =decryptImg(encData, key.getBytes(), true);
			    	//System.out.println("decData.length :: "+ decData.length);
			    	bos.write(decData, 0, decData.length);
		    	}
		    	else{
		    		byte[] decData =decryptImg(encData, key.getBytes(), false);
			    	//System.out.println("decData.length :: "+ decData.length);
			    	bos.write(decData, 0, decData.length);	
		    	}
			}
